...
const ICONS = {
    ...
    notRuoi: <svg xmlns="http://www.w3.org/2000/svg" className={iconClass} fill="currentColor" viewBox="0 0 16 16"><path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z"/><path d="M1.5 1a.5.5 0 0 0-.5.5v2a.5.5 0 0 0 .5.5h2a.5.5 0 0 0 .5-.5v-2a.5.5 0 0 0-.5-.5h-2zM1.5 5a.5.5 0 0 0-.5.5v2a.5.5 0 0 0 .5.5h2a.5.5 0 0 0 .5-.5v-2a.5.5 0 0 0-.5-.5h-2zm0 4a.5.5 0 0 0-.5.5v2a.5.5 0 0 0 .5.5h2a.5.5 0 0 0 .5-.5v-2a.5.5 0 0 0-.5-.5h-2z"/></svg>,
};

const PhysiognomyResult: React.FC<Props> = ({ analysisData, imageData, onTryAgain, onGoHome, onOpenDonationModal }) => {
  ...
  const analysisSections = [
      ...
      ...(analysisData.notRuoiVaTanNhang ? [{ title: t('physiognomyResultSection5'), content: analysisData.notRuoiVaTanNhang, icon: ICONS.notRuoi }] : []),
        ...
        <div className="lg:col-span-2 space-y-8">
            {analysisSections.map((section, index) => (
                <div key={index} className="animate-slide-in-up" style={{ animationDelay: `${index * 150}ms`, opacity: 0 }}>
                    <AnalysisSection title={section.title} content={section.content} icon={section.icon} colorClass="text-yellow-400" />
                </div>
            ))}
        </div>
      </div>
